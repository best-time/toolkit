...
(function (global) {
    ...
    var featureDetect = function () {
        ...
        var len = requiredFeatures.length
        for (var i = 0; i < len; i++) {
    ...
    psQuery.prototype = {
        ...
        init: function (selector) {
            if (!featureDetect()) throw 'Required browser features are not available.';

            try {
                if (!selector) {
                    throw 'Error: Invalid selector';
                } else if (!!selector.nodeType && (selector.nodeType === 1 || selector.nodeType === 9)) { // 元素 / Document
                    this.els = [selector];
                ...
                if (this.length === 0) throw 'Error: No elements found with that selector.';
