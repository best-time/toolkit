;(function (root) {
    ...
    var docUrl = location.href.split('?')[0];//去除问号之后部分 当前页面的url
    var baseUrl = getCurSrc() || docUrl;       //正在执行js的src 路径
    ...
    var o = {}; //存储配置项 {baseUrl, cache, path }
    ...
    function fixUrl(url) { // m//  => m/, 剔除不必要的 '/'
        return url.replace(/([^:])\/+/g, '$1/');
    }
    function getUrl(path, url) {  //根据basePath 拼接 相对路径的js,形成完整的js url路径
        //绝对网址
        if (isUrl(path)) {
            return fixUrl(path);
        }
        ...
        var rootUrl;
        //修复url
        if (rootUrl = url.match(/[^\/]*\/\/[^\/]*\//)) {  // => 获取 // - / 之间的字符
            //http://yanhaijing.com/abc
            url = url.slice(0, url.lastIndexOf('/') + 1); //剔除结尾 '/'
            rootUrl = rootUrl[0];
        } else {
            //http://yanhaijing.com
            rootUrl = url = url + '/';
    ...
    function getDepUrl(id, url) {
        ...
        var rurl = getUrl(pathId, url || o.baseUrl);
        var rres = fixSuffix(rurl, 'js');
        return rres;
    }
    ...
    function define(name, deps, callback) {
        ...
        modeMap =>
        [name: {
         deps:[], callback: callback, status: 'loaded', oncomplete: []
        }]
         moduleMap=>
         [name: {}]
        */
    ...
    function config(option) {
        ...
        //fix keywords
        o.path.BASEURL = fixPath(option.baseUrl || o.baseUrl); //正在执行的js src 路径
        o.path.DOCURL = fixPath(docUrl); //当前页面的url
        return extendDeep({}, o);
    }
    var lodjs = {
        use: use,
