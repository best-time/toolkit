...
;(function(root, factory) {
    ...
    // helper function which call predicate function
    // per parameter and return true if all pass
    function all(func) {
        return function() {
            var parameters = arraySlice.call(arguments);
            var length = parameters.length;
            if(length === 1 && is.array(parameters[0])) {    // ֻ����һ��������� support array
                parameters = parameters[0];
                length = parameters.length;
            }
            for (var i = 0; i < length; i++) {
                if (!func.call(null, parameters[i])) { //��һ����false, �򷵻�false
                    return false;
                }
            }
            return true;
    ...
    // helper function which call predicate function
    // per parameter and return true if any pass
    function any(func) {
        return function() {
            var parameters = arraySlice.call(arguments);
            var length = parameters.length;
            ...
            for (var i = 0; i < length; i++) {
                if (func.call(null, parameters[i])) { //��һ����true,�򷵻�true
                    return true;
                }
            }
            return false;
    ...
    is.empty = function(value) {
        if(is.object(value)){ //
            ...
            if(num === 0 ||
                (num === 1 && is.array(value)) ||
                (num === 2 && is.arguments(value))
            ){
                return true;
            }
            return false;
    ...
    // horizantal tab: 9,
    // line feed: 10, vertical tab: 11, form feed: 12, carriage return: 13, space: 32
    ...
    is.endWith = function(str, endWith) {
        return is.string(str) &&
            str.indexOf(endWith) > -1 &&
            str.indexOf(endWith) === str.length -  endWith.length;
    };
    // endWith method does not support 'all' and 'any' interfaces
    is.endWith.api = ['not'];

    // is a given string or sentence capitalized?
    is.capitalized = function(str) {    // ������ ÿ���λ�  �׵����Ƿ��Ǵ�д
    ...
    is.palindrome = function(str) {     //�ַ�ת�� �� ԭ�ַ�һ��
    ...
    is.leapYear = function(year) { //����ָ�ܱ�4����Ҳ��ܱ�100���, �����ܱ�400���
