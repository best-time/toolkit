...
(function (context) {
    ...
    var timer_start = function () {
        this.id = (this.type == 'i') ?
            setInterval(this.callback, this.when) :
    ...
    function create_timer() {
        ...
        var period = (arguments[0] instanceof Array ?
                arguments[0].join('') :
                String(arguments[0])) + (ac === 3 ? String(arguments[1]) : '');

        if (isNaN(parseInt(period, 10))) {
            period = '1' + period;
        }

        var match = period.toLowerCase().replace(/[^a-z0-9\.]/g, "")
