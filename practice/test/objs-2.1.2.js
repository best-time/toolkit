...
new function() {
	...
	Objs = function( classpath, second, third ) {
		 ...
		 */
		path = $prefix + classpath; //$prefix = "$Objs$"
		...
		if( second == null ) { //第二个参数传入 undefined 或者 null
			...
			throw Error( $NonExistentClass + classpath ); //如果第二个参数传入 undefined 或者 不传参 就抛此处错
		}
		 ...
		 * classpath.
		 */
		if( (secondType = typeof second) == Tstring ) { //第二个参数是字符串, 相当于传入了父类
			//console.log(map);
			//console.log(second)
			//console.log($prefix)
		 ...
		 */
		temp = func.prototype;
		for( propName in protobject ){ //把第二个参数的方法放到func原型中
