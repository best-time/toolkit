...
(function($, window, document, undefined) {
    ...
    */
    ...
    function Plugin(element, options) {
        this.options = $.extend({}, defaults, options); //合并初始化对象
        ...
        this._numPages = Math.ceil(this._items.length / this.options.perPage); // 总页数
        this._currentPageNum = this.options.startPage;   //当前页数
    ...
    Plugin.prototype.setNav = function() {
        ...
        this._holder.each(this.bind(function(index, element) {
            var holder = $(element);
            holder.prepend(navhtml);			//当要添加input框搜索时, 需要 prepend添加分页
    ...
    Plugin.prototype.writeNav = function() {
        ...
        navhtml = this.writeBtn("first") + this.writeBtn("previous"); //首页 + 上一页
        ...
        navhtml += this.writeBtn("next") + this.writeBtn("last") + "</div>";  // 下一页 + 最后一页
    ...
    Plugin.prototype.updateItems = function(page) {
        var range = this.getItemRange(page); // {start:1, end: perPage}
    ...
    $.fn[name] = function(arg) {
        var type = $.type(arg);		//获取数据类型
		console.log(this)
        if (type === "object") {				//传入对象 初始化分页
        ...
        if (type === "string" && arg === "destroy") { //传入任意字符串 或 destroy  摧毁分页
            instance.destroy();
            this.each(function() {
        ...
        if (type === 'number' && arg % 1 === 0) {   //是数字并且是整数
